...
#define sizeDefinitionDeclare "uint32_t %s_size;\n"
#define sizeDefinitionDefine  "uint32_t %s_size = %d;\n"
#define sizeMacroDefinition   "#define %s_size %d\n"

#define typeName              "uint8_t"
...
static void processFile(const char *fileName, long int from, long int to)
{
    ...
    if (headerFile) {
        if (createMacro) {
            ...
            header("extern "sizeDefinitionDeclare, symbolName, to - from + 1);
            output(sizeDefinitionDefine, symbolName, to - from + 1);
        }
    } else {
        if (createMacro) {
